...
#_________________________________________________________________________________________________
#
# This script spawns all speech and image negative pair generation for all specified speech and image 
...
def main():
    ...
    for (fn, num, metr) in list(itertools.product(feat_fns, NUM_PAIRS, METRIC)):  
        ...
        cmd = "./classifier_negatives.py " + " --feats_fn {} --num_pairs {} --metric {}".format(fn, num, metr)
        print_string = cmd
        model_setup_library.command_printing(print_string)
        sys.stdout.flush()
        proc = subprocess.Popen(cmd, shell=True)
