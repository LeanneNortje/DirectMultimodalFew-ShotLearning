...
#_________________________________________________________________________________________________
#
# This script generates speech and image negative pairs from latents extracted from a classifier 
#model.
#
...
def main():
    ...
    key_pair_file = path.join(key_pair_file, "key_" + model + "_negatives.list")
    ...
    lower_distance_limit = 0.6
    upper_distance_limit = 0.8

    lower_limit = int(0.50*latents.shape[0])
    upper_limit = int(0.70*latents.shape[0])

    valid_counts = np.arange(lower_limit, upper_limit, 1)
    np.random.seed(1)
    ...
    for i in tqdm(range(len(latents)), desc="Calculating distances", ncols=COL_LENGTH):
        ...
        if speech_not_image_pairs is False:
            if np.logical_and(distances > lower_distance_limit, distances < upper_distance_limit).any() is False:  continue 
        pairs = []

        
        np.random.shuffle(valid_counts)
        counter = 0

        while len(pairs) < args.num_pairs and counter < len(valid_counts) -1:
            count = valid_counts[counter]
            pair_key = keys[indices[count]]
            
            if current_key != pair_key: 
                
                if speech_not_image_pairs and current_key.split("_")[1].split("-")[0] == pair_key.split("_")[1].split("-")[0]:
                    pairs.append(pair_key)
                elif speech_not_image_pairs is False and distances[count] > lower_distance_limit and distances[count] < upper_distance_limit: 
                    pairs.append(pair_key)
